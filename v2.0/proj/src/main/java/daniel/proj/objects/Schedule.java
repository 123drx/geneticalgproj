...
//import daniel.proj.Servieces.ScheduleServies;
...
public class Schedule {
    ...
    public String[][] getScheduleasarr() {
        String[][] scheduleArray = new String[MaxDays][MaxHours+1];
        
        // Populate the schedule array
        for (int day = 0; day < MaxDays; day++) {
            for (int hour = 0; hour < MaxHours+1; hour++) {
                if(hour ==0)
                {
                    scheduleArray[day][0] = daysOfWeekMap.get(day);
                }
                else{
                Lesson currentLesson = Schedual[day][hour-1];
                if (currentLesson.getLessonSubject() != null) {
                    // Store lesson information
                    if (currentLesson.getTeacher() != null) {
                        scheduleArray[day][hour] = currentLesson.getLessonSubject() + "," + currentLesson.getTeacher();
                    } else {
                        scheduleArray[day][hour] = currentLesson.getLessonSubject();
                    }
                } else {
                    // Store empty slot
                    scheduleArray[day][hour] = "--------";
                }
            }
        }
        }
